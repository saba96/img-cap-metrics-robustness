...
def main():
    ...
    for id in negated_correct_caps:
        ...
        incorrect_scores.append(negated_correct_caps[id]['PAC-S'])
        score = (correct_caps[id]['PAC-S'], negated_correct_caps[id]['PAC-S'])
        if np.abs(correct_caps[id]['PAC-S'] - negated_correct_caps[id]['PAC-S']) == 0:
            almost_equal_correct += 1
            #print('sample correct:', id)
        all_scores.append(score)
        if (correct_caps[id]['PAC-S'] < negated_correct_caps[id]['PAC-S']):
            incorrect_ranking += 1
