...
def main():
    with open('../dataset/fine_grained/filtered_correct_clip_score.json') as f:
        correct_caps = json.load(f)
    with open('../dataset/fine_grained/filtered_incorrect_clip_score.json') as f:
    ...
    for id in incorrect_caps:
        ...
        incorrect_scores.append(incorrect_caps[id]['CLIPScore'])
        score = (correct_caps[id]['CLIPScore'], incorrect_caps[id]['CLIPScore'])
        if np.abs(correct_caps[id]['CLIPScore'] - incorrect_caps[id]['CLIPScore']) == 0:
            almost_equal_correct += 1
            #print('sample correct:', id)
        all_scores.append(score)
        if (correct_caps[id]['CLIPScore'] <= incorrect_caps[id]['CLIPScore']):
            incorrect_ranking += 1
