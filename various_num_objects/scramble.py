...
def scramble(sentence1):
    ...
    shuffle(order) 
    #print(order)
    while ((order==np.arange(0, min_length)).all() and (order[:2]==np.arange(0, 2)).all()):
        shuffle(order) 
        #print(order)           
    sentence1_shuffled =  ''
    for index in order:
...
def pick_tags(image_tags, categories):
    ...
    with open('../dataset/various_num_objects/one_tag_unique.json') as f:
       one_tag = json.load(f)
    with open('../dataset/various_num_objects/two_tags_unique.json') as f:
       two_tags = json.load(f)
    with open('../dataset/various_num_objects/three_tags_unique.json') as f:
       three_tags = json.load(f)
    one_tag_caps_shuffled = []
    two_tags_caps_shuffled = []
    three_tags_caps_shuffled = []
    for cap in one_tag:
        cap['caption'] = scramble(cap['caption'])
        cap['generation_mode'] = 'one_tag_shuffled'
        one_tag_caps_shuffled.append(cap)
    for cap in two_tags:
        cap['caption'] = scramble(cap['caption'])
        cap['generation_mode'] = 'two_tags_shuffled'
        two_tags_caps_shuffled.append(cap)
    for cap in three_tags:
        cap['caption'] = scramble(cap['caption'])
        cap['generation_mode'] = 'three_tags_shuffled'
        three_tags_caps_shuffled.append(cap)
    with open('../dataset/various_num_objects/one_tag_shuffled_captions.json', 'w') as f:
        json.dump(one_tag_caps_shuffled, f)   
    with open('../dataset/various_num_objects/two_tags_shuffled_captions.json', 'w') as f:
        json.dump(two_tags_caps_shuffled, f)
    with open('../dataset/various_num_objects/three_tags_shuffled_captions.json', 'w') as f:
        json.dump(three_tags_caps_shuffled, f)
...
def main():
    ...
    #with open('/home/mila/s/saba.ahmadi/scratch/umic/umic_output/postprocessed/ALBEF_success_results.json') as f:
    #    vqa_caps = json.load(f)
    print(categories)
